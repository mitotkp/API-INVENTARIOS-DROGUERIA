...
export class  inventoryQuerys {
    ...
    static closePedido = `
        BEGIN TRY
            ...
            DECLARE @ID_DEL_PEDIDO VARCHAR(20);
            SELECT @ID_DEL_PEDIDO = IDPEDIDO FROM DBO.PEDIDOS_CONTEOS WHERE IDCONTEO = @IDCONTEO;

            UPDATE DBO.PEDIDOS_CONTEOS
            SET ESTADO = 'CERRADO' , ESTADOPED = 'OK'
    ...
    static deleteBulto = `
        BEGIN TRY
            ...
            UPDATE C
            SET C.UNIDADES = CASE WHEN (C.UNIDADES - B.UNIDADES) < 0 THEN 0 ELSE (C.UNIDADES - B.UNIDADES) END
            FROM DBO.CONTEOSLIN C
            INNER JOIN DBO.BULTOS_CONTEO B ON C.IDCONTEO = B.IDCONTEO AND C.CODARTICULO = B.CODARTICULO
            WHERE B.IDCONTEO = @IDCONTEO AND B.IDBULTO = @IDBULTO AND B.CODARTICULO IS NOT NULL;

          
            INSERT INTO DBO.BULTOS_CONTEO_ELIMINADOS (IDPEDIDO, IDCONTEO, IDBULTO, CODARTICULO, TALLA, COLOR, IDTARIFAV, PRECIOUNITARIO, UNIDADES)
            SELECT IDPEDIDO, IDCONTEO, IDBULTO, CODARTICULO, TALLA, COLOR, IDTARIFAV, PRECIOUNITARIO, UNIDADES
            FROM DBO.BULTOS_CONTEO
            WHERE IDCONTEO = @IDCONTEO AND IDBULTO = @IDBULTO;
